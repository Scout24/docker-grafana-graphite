...
RUN     apt-get -y install software-properties-common cron
...
RUN     mkdir /src/grafana                                                                                    &&\
        mkdir /opt/grafana                                                                                    &&\
        wget https://grafanarel.s3.amazonaws.com/builds/grafana-3.0.3-1463994644.linux-x64.tar.gz -O /src/grafana.tar.gz &&\
...
RUN echo "#!/bin/bash" > /etc/cron.daily/purge-wisper
RUN echo "find /vol/monitoring/data/whisper/ -mtime +30 -exec rm -rf {} \;" >> /etc/cron.daily/purge-wisper
RUN chmod +x /etc/cron.daily/purge-wisper
